...
REQUIRED_ARTIFACTS=(
  ...
  "models/base_runtime/pre_artifact/hand_model.npz"
  "models/base_runtime/pre_artifact/chunk_aggregator_model.npz"
  "models/base_runtime/pre_artifact/score_chunk.py"
  "models/base_runtime/pre_artifact/feature_extractor_frozen.py"
...
missing_optional="$($VENV_BIN/python - <<'PY'
import json
from pathlib import Path
repo = Path.cwd()
base_root = repo / 'models'
config = json.loads((repo / 'models' / 'base_runtime' / 'synth_manifest.json').read_text(encoding='utf-8'))
missing = []
for key in ['pf_penalty_artifact_dir', 'pf_boost_artifact_dir', 'post_penalty_artifact_dir', 'post_boost_artifact_dir']:
...
if [[ -n "$missing_optional" ]]; then
  echo "ERROR: Missing model artifact(s):"
