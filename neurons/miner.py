...
class Miner(BaseMinerNeuron):
    ...
    def __init__(self, config=None):
        ...
        self.model_manifest = build_local_model_manifest(
            ...
            implementation_files=[
                ...
                repo_root / "models" / "base_runtime" / "synth_manifest.json",
                repo_root / "models" / "base_runtime" / "pre_artifact" / "hand_model.npz",
                repo_root / "models" / "base_runtime" / "pre_artifact" / "chunk_aggregator_model.npz",
                repo_root / "models" / "base_runtime" / "pre_artifact" / "score_chunk.py",
                repo_root / "models" / "base_runtime" / "pre_artifact" / "feature_extractor_frozen.py",
                *[
                    path for path in [
                        repo_root / "models" / "base_runtime" / "pf_penalty_artifact" / "model.json",
                        repo_root / "models" / "base_runtime" / "pf_boost_artifact" / "model.json",
                        repo_root / "models" / "base_runtime" / "post_penalty_artifact" / "model.json",
                        repo_root / "models" / "base_runtime" / "post_boost_artifact" / "model.json",
                    ] if path.exists()
                ],
                repo_root / "export_benchmark_phase1_dataset.py",
                repo_root / "gen17_math_1_pf.py",
                repo_root / "gen17_math_1_pf_boost.py",
                repo_root / "gen17_math_1_post.py",
                repo_root / "gen17_math_1_post_boost.py",
            ...
            defaults={
                ...
                "license": "MIT",
                "repo_url": "https://github.com/tomkaba/poker44-miner-gen17-tuner-synth15",
                "repo_commit": git_commit,
                "notes": "Overlay tuner fitted on labeled public benchmark chunks on top of the gen17_synth15 base synth runtime.",
                "open_source": True,
                "inference_mode": "local",
                "training_data_statement": "Uses the gen17_synth15 base synth artifact with a recall-biased overlay tuner fitted on labeled public benchmark chunks.",
                "private_data_attestation": "This artifact does not introduce validator-private human data.",
                "data_attestation": "This artifact combines benchmark-derived frozen synth artifacts with an overlay tuner fitted on public benchmark labels.",
            },
        )
    ...
    async def forward(self, synapse: DetectionSynapse) -> DetectionSynapse:
        ...
        bt.logging.info(f"Scored {len(chunks)} chunks with scorer gen17_tuner_synth15.")
        return synapse
...
if __name__ == "__main__":
    ...
    with Miner() as miner:
        ...
        while True:
            bt.logging.info(
                f"Miner UID: {miner.uid} | Incentive: {float(miner.metagraph.I[miner.uid])} | Scorer: gen17_tuner_synth15"
            )
            time.sleep(60)
